...
Filter_person <- person %>%
  filter(
    grepl("Annabel", name, ignore.case = TRUE),
    grepl("Franklin Ave",address_street_name, ignore.case = TRUE))
...
interview_filtered <- interview %>%
  filter(person_id %in% merged_suspect$id)%>%
  left_join(merged_suspect, by = c("person_id"= "id"))
...
```{r}
membership_filtered <- get_fit_now_member %>%
  filter(membership_status == "gold" , grepl("48Z", id))
```
 
```{r}
checkin_filtered<-get_fit_now_check_in %>%
  filter(
    membership_id == "48Z7A"| membership_id== "48Z55",
...
```{r}
suspected_plate<- drivers_license %>%
...
```{r}
car_match <- person %>%
  filter(license_id ==423327| id== 664760)

Killer<-car_match$name
cat("Killer is:", Killer)

#Killer is Jeremy Bowers
```

```{r}
interview_jeremy<- interview%>%
  filter(person_id %in% car_match$id)

interview_jeremy$transcript
```
```{r}
mastermind_details <- drivers_license%>%
  filter(hair_color == "red",car_make == "Tesla", car_model=="Model S", gender == "female", height>=65, height<=67)
```


```{r}
mastermind_event <- facebook_event_checkin%>%
  filter(event_name=="SQL Symphony Concert", grepl(201712, date))%>%
  group_by(person_id)%>%
  filter(n()==3)%>%
  ungroup()
...
```{r}
mastermind_name_id<- person%>%
  filter(license_id %in% mastermind_details$id)

```


```{r}
Mastermind<- mastermind_name_id%>%
  filter(id %in% mastermind_event$person_id)

cat("Masterman found: Masterman is actually woman:",Mastermind$name, "\nMission Accomplished!!! \nOver and Out")
